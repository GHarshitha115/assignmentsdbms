...
Q7="SELECT fname,lname FROM MovieDirector INNER JOIN Director ON `MovieDirector`.did=`Director`.id INNER JOIN Movie ON `MovieDirector`.mid=`Movie`.id WHERE name Like 'Star Trek%'"
Q8='''SELECT name FROM Movie
INNER JOIN Cast ON 
`Movie`.id=`Cast`.mid 
INNER JOIN Actor ON 
 `Actor`.id=`Cast`.pid
INNER JOIN MovieDirector ON
`Movie`.id=`MovieDirector`.mid
INNER JOIN Director ON
`Director`.id=`MovieDirector`.did
WHERE Actor.fname='Jackie (I)' AND Actor.lname='Chan' AND
Director.fname='Jackie (I)' AND Director.lname='Chan'
'''
Q9="SELECT fname,lname FROM Director INNER JOIN MovieDirector ON `MovieDirector`.did=`Director`.id INNER JOIN Movie ON `MovieDirector`.mid=`Movie`.id WHERE year = 2001 GROUP BY did HAVING COUNT(mid)>=4 ORDER BY fname ASC,lname DESC"
Q10="SELECT gender,Count(gender) FROM Actor GROUP BY gender ORDER BY gender ASC"
Q11='''
SELECT DISTINCT m1.name,m2.name,m1.rank,m1.year FROM Movie AS m1,Movie AS m2 
WHERE (m1.year=m2.year AND m1.rank=m2.rank) AND m1.name<>m2.name
ORDER BY m1.name ASC Limit 100
'''
Q12='''
SELECT fname,year,AVG(rank) FROM Cast INNER JOIN Actor ON 
`Actor`.id=`Cast`.pid INNER JOIN Movie ON `Movie`.id=`Cast`.mid 
GROUP BY year,`Actor`.id ORDER BY fname ASC,year DESC Limit 100;
'''
Q13='''
SELECT Actor.fname,Director.fname,AVG(rank) as score FROM 
Actor INNER JOIN Cast ON
`Cast`.pid=`Actor`.id
INNER JOIN Movie ON
`Cast`.mid=`Movie`.id
INNER JOIN MovieDirector ON 
`MovieDirector`.mid=`Movie`.id
INNER JOIN Director ON
`Director`.id=`MovieDirector`.did
