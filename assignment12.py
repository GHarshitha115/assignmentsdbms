...
class Student:
    ...
    def get(sql):
        import sqlite3
        #connection=sqlite3.connect("dbms/dbms_resources/students_db.sqlite3")
        connection=sqlite3.connect("dbms/dbms_submissions/dbms_assignment_012/db.sqlite3")
        crsr=connection.cursor()
        crsr.execute("SELECT * from Student where student_id=3;")    
        ans=(crsr.fetchone())
        b=Student(*ans)
        connection.close()
        return b
student_obj=Student.get("student_id=2")
print(student_obj.name)
